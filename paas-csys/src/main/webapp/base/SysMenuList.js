...
//	var TF_f2 = EF.getComboBox({fieldLabel:"菜单类型",hiddenName:"menuType",width:comwidth,def:"V_SYS_MODU_TYPE",allowBlank:false});
	...
	//树
	TR_menu = EF.getTreePanel({tbar:[B_addNode,B_addChildNode],border:true,layout:"fit",collapseMode:"mini",split:true,region:"west",width:360,root:EF.getTreeRoot({id:"00"}),url:"/base/sysmenu/querySysMenuTree?menuType=1"});
...
function showSelectModu() {
	if(W_selectModuWin == null) {
		W_moduTree = EF.getTreePanel({border:false,layout:"fit",root:EF.getTreeRoot({id:"00"}),url:"/base/sysmodu/querySysModuTree?moduType=1"});
